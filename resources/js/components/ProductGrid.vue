      ...
      <div class="showcase" v-for="product in products" :key="product.id">
        <div class="showcase-banner">
          ...
          <router-link :to="{ path: '/product/' + product.id }">
            <img src="template/user/assets/images/products/jacket-4.jpg" alt="{{ product.name }}"
              class="product-img default">
            <img src="template/user/assets/images/products/jacket-4.jpg" alt="{{ product.name }}" width="300"
...
export default {
  ...
  methods: {
    ...mapActions(['addToCart', 'setCart', 'fetchWatchlist']),
      ...
      axios.post('/api/watchlist', { product_id: productId, quantity: quantity })
        .then(response => {
          if (response.data && response.data.status === 'success') {
            ...
            this.fetchWatchlist(); // Update the watchlist
          } else {
